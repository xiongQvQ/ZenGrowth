...
"""
测试监控和日志增强功能
验证提供商特定的指标收集、详细的请求/响应日志记录和性能监控
"""
...
def test_monitoring_integration():
    ...
    try:
        ...
        # 获取监控统计
        monitoring_stats = provider_manager.get_monitoring_stats()
        
        print(f"监控统计数据类型: {type(monitoring_stats)}")
        print(f"包含的键: {list(monitoring_stats.keys())}")
        
        # 检查系统健康状态
        system_health = monitoring_stats.get('system_health', {})
        print(f"系统健康状态: {system_health.get('status', 'unknown')}")
        print(f"整体成功率: {system_health.get('overall_success_rate', 0):.1%}")
        
        # 检查提供商统计
        provider_stats = monitoring_stats.get('provider_stats', {})
        print(f"监控的提供商数量: {len(provider_stats)}")
        
        for provider, stats in provider_stats.items():
            print(f"  {provider}: {stats.total_requests} 请求, {stats.success_rate:.1%} 成功率")
