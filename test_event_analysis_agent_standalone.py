...
from agents.event_analysis_agent_standalone import (
    EventAnalysisAgent,
...
class TestKeyEventIdentificationTool:
    ...
    def test_key_event_identification_success(self):
        """测试成功的关键事件识别"""
        # 准备测试数据
        test_data = self.create_test_key_event_data()
        self.storage_manager.store_events(test_data)
        ...
        # 验证具体结果
        results = result['results']
        assert len(results) > 0  # 应该有关键事件结果
        
        # 检查结果结构
        for key_event in results:
            assert 'event_name' in key_event
            assert 'importance_score' in key_event
            assert 'user_engagement_impact' in key_event
            assert 'conversion_impact' in key_event
            assert 'retention_impact' in key_event
            assert 'reasons' in key_event
...
class TestEventAnalysisAgent:
    ...
    def test_analyze_event_trends(self):
        """测试事件趋势分析方法"""
        # 创建测试数据
        test_data = pd.DataFrame([
            {
    ...
    def test_analyze_event_correlations(self):
        """测试事件关联性分析方法"""
        # 创建测试数据
        test_data = pd.DataFrame([
            {
    ...
    def test_identify_key_events(self):
        """测试关键事件识别方法"""
        # 创建测试数据
        test_data = pd.DataFrame([
            {
