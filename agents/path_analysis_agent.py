...
该模块实现PathAnalysisAgent类，负责用户行为路径分析和流程优化。
智能体集成了路径分析引擎，提供会话重构、路径挖掘和用户体验优化建议功能。
"""
...
class SessionReconstructionTool(BaseTool):
    ...
    description: str = "从用户事件数据中重构完整的用户会话，基于时间间隔分割会话边界"
...
class PathMiningTool(BaseTool):
    ...
    description: str = "从用户会话中挖掘常见路径模式、异常模式和转化路径"
...
class UserFlowAnalysisTool(BaseTool):
    """用户流程分析工具"""
    
    name: str = "user_flow_analysis"
    description: str = "分析用户行为流程，生成流程图和用户体验优化建议"
...
class PathAnalysisAgent:
    ...
    def __init__(self, storage_manager: DataStorageManager = None):
        ...
        self.tools = [
            SessionReconstructionTool(storage_manager),
            PathMiningTool(storage_manager),
            UserFlowAnalysisTool(storage_manager)
        ]
        
        if CREWAI_AVAILABLE:
            self.agent = Agent(
                role="用户体验分析师",
                goal="分析用户行为路径，识别用户流程模式，提供用户体验优化建议",
                backstory="""你是一位专业的用户体验分析师，专注于用户行为路径分析和流程优化。
                            你擅长从用户的行为数据中重构完整的用户旅程，识别常见的用户路径模式
                            和异常行为。你的分析帮助产品团队理解用户如何与产品交互，
                            发现用户体验中的痛点和优化机会。""",
                tools=self.tools,
                llm=get_llm(),
                verbose=True,
    ...
    def reconstruct_sessions(self, user_ids: Optional[List[str]] = None,
        ...
        """
        try:
            session_tool = SessionReconstructionTool(self.storage_manager)
            return session_tool._run(user_ids, date_range, session_timeout_minutes)
        except Exception as e:
            logger.error(f"会话重构失败: {e}")
            return {
                'status': 'error',
    ...
    def mine_path_patterns(self, sessions: Optional[List[Dict]] = None,
        ...
        """
        try:
            mining_tool = PathMiningTool(self.storage_manager)
            return mining_tool._run(sessions, min_length, max_length, min_pattern_frequency)
        except Exception as e:
            logger.error(f"路径挖掘失败: {e}")
            return {
                'status': 'error',
                'error_message': str(e),
                'analysis_type': 'path_mining'
    ...
    def analyze_user_flow(self, analysis_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        try:
            flow_tool = UserFlowAnalysisTool(self.storage_manager)
            return flow_tool._run(analysis_result)
        except Exception as e:
            logger.error(f"用户流程分析失败: {e}")
            return {
                'status': 'error',
